...
describe("ResponderAppointmentReducer", () => {
    ...
    fnIt<ResponderAppointmentReducer>(
        r => r.reduce,
        "transitions from pending to mined",
        async () => {
            const reducer = new ResponderAppointmentReducer(blockCache, txID1.identifier, appointmentId1, 0, from1);
    ...
    fnIt<ResponderAppointmentReducer>(
        r => r.reduce,
        "doesn't mine tx from different address",
        async () => {
            const reducer = new ResponderAppointmentReducer(blockCache, txID1.identifier, appointmentId1, 0, from1);
...
describe("MultiResponderComponent", () => {
    ...
    fnIt<MultiResponderComponent>(
        m => m.detectChanges,
        "detects response has been mined",
        async () => {
            const app1State = makePendingAppointmentState("app1", "data1");
    ...
    fnIt<MultiResponderComponent>(
        m => m.detectChanges,
        "doesnt detect already mined response",
        async () => {
            const app2State = makeMinedAppointmentState("app1", "data1", 0, 0);
    ...
    fnIt<MultiResponderComponent>(
        m => m.detectChanges,
        "removes item after confirmations",
        async () => {
            const app2State = makeMinedAppointmentState("app1", "data1", 0, 0);
            const state1 = setupState([app2State], 0);
            // two block difference
    ...
    fnIt<MultiResponderComponent>(
        m => m.detectChanges,
        "does not try to remove already removed item",
        async () => {
            const app2State = makeMinedAppointmentState("app1", "data1", 0, 0);
