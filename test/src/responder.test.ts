...
describe("EthereumMultiResponder", () => {
    let ganache;
    let provider: ethers.providers.Web3Provider;
    ...
    it("emits the AttemptFailed and ResponseFailed events the correct number of times on failure", async () => {
        ...
        // Make sendTransaction return promises that throws an error
        sinon.replace(signer, 'sendTransaction', () => Promise.reject(new Error("Some error.")));
