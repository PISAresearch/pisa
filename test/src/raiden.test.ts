...
describe("Raiden end-to-end tests for scenario 2 (with Pisa)", function() {
    ...
    beforeEach(async () => {
        ...
        //Start parity node
        parity = exec(
            `parity --config dev --base-path ${demoDir}/copiedChainData --chain ${demoDir}/docker/test-chain.json --jsonrpc-interface 0.0.0.0 --jsonrpc-port 8545 --jsonrpc-apis=eth,net,web3,parity --network-id 3`
        );
