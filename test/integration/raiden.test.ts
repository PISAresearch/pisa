...
describe("Raiden end-to-end tests for scenario 2 (with Pisa)", function() {
    ...
    beforeEach(async () => {
        ...
        pisa = exec(`ts-node ${pisaRoot}/packages/main/src/startUp.ts --db-dir=${demoDir}/${pisaDbDir} --json-rpc-url=http://localhost:8545 --host-name=0.0.0.0 --host-port:3000 --responder-key=${responderKey} --pisa-contract-address=${pisaContract.address} --instance-name=pisa-raiden`); //prettier-ignore
