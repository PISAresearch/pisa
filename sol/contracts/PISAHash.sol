...
contract PISAHash {
    ...
    function didPISARespond(bytes32 _pisaid, uint _jobid, bytes32 _expectedHash, uint[3] memory _timewindow) internal returns (bool) {
    ...
    function computeAppointment(bytes memory _appointment) internal pure returns(Appointment memory) {
        ...
        (appointment.appointmentid, appointment.jobid, appointment.startTime, appointment.finishTime, appointment.challengeTime, appointment.refund, appointment.h) = abi.decode(appointmentinfo, (bytes32, uint, uint, uint, uint, uint, bytes32));
